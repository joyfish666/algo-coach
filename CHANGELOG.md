...
- Statement rendering was corrupted on most problems (literal `************` runs before
  every example block, `**` leaking around bold segments like `**进阶：**你`). Two root
  causes: the HTML→Markdown converter emitted `**` markers for `<strong>` tags INSIDE
  `<pre>` blocks into the paragraph buffer (cn wraps the 输入：/输出： labels in bold), and
  CommonMark's flanking rules refuse to close a delimiter run preceded by punctuation and
  followed by a CJK letter (or when whitespace sits inside the wrap). The converter now
  treats in-fence tags as literal text and rebuilds bold/italic segments with
  flanking-safe emission (verified end-to-end through markdown-it: zero leaked asterisks).
- AI coach chat bubbles interpolated the LLM reply as plain text while the analytics
  report already rendered markdown - the coach's replies showed literal `**bold**`,
  list dashes and code fences. Assistant bubbles now render markdown (html escaped,
  chat-style line breaks); user drafts and error notices stay verbatim, and the history
  fed back to the model keeps the raw text.
- Stored statements generated by the old converter could never heal themselves;
  `meta.json` now records a `statement_version` and opening a problem lazily regenerates
  the statement online once (offline degrades to the stored file, keeping offline review).
- PITFALLS carried two drifted duplicate "前端与服务" sections; merged into one union copy.
