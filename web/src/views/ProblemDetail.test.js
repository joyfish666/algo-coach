...
describe('workbench layout', () => {
  ...
  it('dragging the main divider resizes the left pane and persists', async () => {
    ...
    leftEl.getBoundingClientRect = () => ({ left: 0, width: 1000, top: 0, height: 800 })
  ...
  it('dragging the horizontal divider resizes the editor zone', async () => {
    ...
    rightEl.getBoundingClientRect = () => ({ left: 0, width: 500, top: 0, height: 1000 })

    await wrapper.find('[data-testid="divider-editor"]').trigger('mousedown')
    expect(document.body.style.cursor).toBe('row-resize')

    document.dispatchEvent(new MouseEvent('mousemove', { clientX: 0, clientY: 800 }))
    await flushPromises()
