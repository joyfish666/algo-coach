...
- **表格行不是段落，`<code>` 里的强调不可渲染**（2026-08-30 已修复）：
  两个独立根因都表现为「渲染出裸文本/裸星号」。①每个 `<tr>` 被当成独立段落 flush，
  且标签间空白文本节点让首单元格先吃进一个 `|` 分隔——产出「行首多竖线、行尾无竖线、
  无分隔行、行间空行」的伪表格，markdown-it 只能整段按字面渲染（如颠倒二进制位的
  整数/二进制表）。必须发射完整 GFM 管道行 + 表头后补 `| --- |` 分隔行，行间用单换行
  连接；单元格内的 `|` 转义为 `\|`（GFM 在 code span 内也认 `\|`），单元格内的块级
  标签一律内联，否则会把一行撕成多段。②`<code><strong>1</strong></code>` 把 `**`
  发射进了反引号内部，code span 里一切标记按字面显示（「仅包含数字 **1**」）。
  强调若开在行内 code span 之内，只能丢弃标记（code 字体本身已有强调效果），
  丢弃时别忘了删掉开始时已写入缓冲的开标记。表格边框同理是前端职责：markdown-it
  产出的是无边框 `<table>`，渲染容器必须自带网格线样式。
- **statement.md 是「转换器版本化的可再生文件」**：`meta.json` 记录
  `statement_version`，转换器输出形状变化时递增 `lc.workspace.STATEMENT_VERSION`；
...
- **i18n catalog 的完整性必须双向校验，死键与缺键同样有害**（2026-08-29 已修复）：
  缺键会让 `t()` 静默回显裸键，而「只增不删」的反面——功能演进后无人回收的**死键**——会静默
  腐化：`setup_body` 在向导交付后仍声称「向导后续版本提供，先用 API」，与产品事实直接矛盾。
  check:i18n 原本只查 used ⊆ catalog 单向，死键永不失败。反向可达性检查必须承认四类合法来源：
  字面 `t('...')`、`titleKey/labelKey/key` 对象字面量（路由 meta/ThemeSwitch/toast 载荷）、
  **显式声明**的动态键族（`verdict_`/`diff_`，各注明构造点）、以及从 `lc/i18n.py` 解析出的
  服务端驱动键（后端侧由 `tests/test_i18n.py` 的镜像护栏覆盖：每个 message_key 必须在
  lc/server 源码中被真实 raise，否则同样视为死键删除）。
- **LLM 提示词必须跟随 UI 语言，且上下文标签要与提示词同语言**（2026-08-29 已修复）：
  应用交付了双语界面，但 coach 系统提示词是硬编码中文常量——英文用户永远收到中文回答。
  端点自己不知道 UI 语言，必须由前端 api 层统一附带（`ui_lang`）。只翻译提示词还不够：
  上下文标签（题目/判定/代码）、分析摘要、指令、用户回合若仍留在另一种语言，模型会被
  拉回旧语言——整个 system prompt 必须单一语言成稿。未知 `ui_lang` 回退 zh（历史行为，
  也是题面所在语言）。
...
- **LLM 的文本输出一律是 Markdown，展示界面必须渲染**（2026-08-28 已修复）：
  OpenAI 兼容模型的回复天然带 `**粗体**`/列表/围栏代码块，纯文本插值会把标记原样漏给
  用户。分析页报告渲染了 markdown-it、聊天侧边栏却没渲染——同类内容两套呈现必然漂移。
  渲染姿势：`html: false`（转义模型输出中的一切 HTML）后 v-html；用户自己的输入和
  错误提示保持纯文本；喂回 LLM 的 history 必须存原始 Markdown 而非渲染后的 HTML。
...
- **测试默认必须与真实数据目录物理隔离**（2026-08-30 事故，已加安全网）：
  拆分 `test_batch6_regressions.py` 时漏掉了原文件的 `isolated_env` fixture，
  其中的「清除数据」回归测试把真实的 `~/.algocoach` 当成沙箱执行了
  `DELETE /api/local-data`，用户的全部本地数据（配置、题库缓存、提交归档、
  收藏、工作区笔记与草稿）被清空且不可恢复。教训有两层：
  ① 涉及数据目录的测试在文件级必须有 `ALGOCOACH_HOME` 隔离——**移动/拆分测试时
  fixture 属于契约的一部分，不是样板**；
  ② 依赖「每个文件都记得隔离」太脆弱，conftest 现在为所有测试默认把
  `ALGOCOACH_HOME` 指到临时目录（文件级 fixture 在其后实例化、可覆盖），
  让这一类事故在机制上不可能再发生。今后任何绕过该安全网的测试都必须在
  PR 里说明理由。
- **手工/E2E 验证同样必须隔离**（2026-08-30 第二次事故，同日）：
  conftest 安全网只覆盖 pytest 进程；为做 UI 截图验证，手工把假 Cookie 与
  合成题库**直接写进真实数据目录**再起服务，随后清理时删掉了用户当天刚重新
  配置的 Cookie 与刚同步的题库。规则：**任何验证用的 coach 实例都必须以
  `ALGOCOACH_HOME=<临时目录>` 启动**（如
  `ALGOCOACH_HOME=$(mktemp -d) python cli.py --no-browser`），真实数据目录
  永远只属于用户的手；需要复现「大量数据」场景时，把合成数据写进临时目录，
  绝不写进 `~/.algocoach`。
