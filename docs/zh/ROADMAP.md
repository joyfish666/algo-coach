...
| UI | AI 教练聊天气泡把 LLM 回复按纯文本插值，`**粗体**`/列表/代码围栏原样漏出（用户实测） | LLM 文本输出一律是 Markdown，而同类内容只有分析页报告做了渲染——两套呈现必然漂移 → 助手气泡经 markdown-it 渲染（html 转义、chat 换行模式），用户输入与错误提示保持纯文本，回传模型的 history 仍存原始文本 |
