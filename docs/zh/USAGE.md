...
每 10 秒向服务发送一次心跳，心跳停止 2 分钟后（等同步完成才退出）服务
自动关闭、窗口随之消失。2 分钟刻意长于浏览器对后台标签约 1 次/分钟的
定时器节流，页面挂在后台不会误杀（回到页面还会立即补一次心跳）；
被浏览器彻底冻结/丢弃的标签仍会触发退出，此时刷新页面即可重新连接，
服务已退出就再双击一次 `start.bat`。退出原因会写入 coach.log（搜
"idle exit"）。手动 `coach` 启动默认不启用该行为，需要时用
`coach --idle-exit 分钟数` 开启。
